        ...
        for chromosome in chromosomes:
            if random.randint(1, 100) <= probability:
                ...
                if count < num_elitism:
                    fases_disponiveis = chromosome.disciplinasRepository.get_fases_disponiveis_por_curso(course)
                    for fase in fases_disponiveis:
                        chromosome_by_phase = chromosome.get_genes_by_phase(fase)
                        mutation_index_com_excesso = chromosome.disciplinasRepository.get_genes_com_excesso_de_carga_horaria(
                ...
                disciplinas_disponiveis = (chromosome.disciplinasRepository
                                           .get_disciplinas_por_curso_fase_e_dia(course, phase,day))

                chromosome_by_phase = chromosome.get_genes_by_phase(phase)
                disciplinas_faltantes = (chromosome.disciplinasRepository
